...
def limpar_fundo_e_pegar_cor(img):
    # Converte para RGBA para ter transparência
    img = img.convert("RGBA")
    data = np.array(img)
    
    # Identifica pixels muito claros (fundo branco/cinza) e os torna transparentes
    # Isso é muito mais leve que usar IA para remover fundo
...
async def processar_layout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if legenda.strip().upper() == "TEMA" and foto:
        try:
            ...
            # 2. Limpar fundo e obter cor da borda
            ...
            # Desenha o contorno de 5mm com a cor do logo, fundo interno branco
